...
async def _refresh_daily_wife_record(
    ev: Event, user_id: str | int | None = None, mode: str = 'wife'
) -> WifeRecord | None:
    """重复抽取：无视历史状态，重新随机选一位并覆盖今天的记录。

    用于「每日老婆/jrlp 可多次抽取」：每次调用都会忽略当天已持有的
    记录（含被抢、送出、离婚等状态），用带时间戳的随机源重新抽一位
    新角色并整体覆盖写回，实现“每次发送都换成新老婆”。
    """
    bucket = _daily_bucket_name(mode)
    key = _user_key(ev, user_id)
    rng = _refresh_rng(ev, key, mode)

    chosen: WifeRecord | None = None
    if mode == 'wife':
        chosen = await _roll_group_member_wife(ev, key, rng)
    if chosen is None:
    ...
    if mode == 'wife':
        # 重新抽取后旧补偿记录（safe_wife）不再有意义，一并清理
        deletes.append(('safe_wives', key))

    async with _daily_context_lock(ev):
        context = await _load_daily_context(ev)
        value = _record_to_dict(chosen, ev, key)
        await _save_daily_records(ev, [(bucket, key, value)], deletes)
    logger.info(f'{LOG_PREFIX} 为用户 {key} 刷新 {mode}: {chosen.name}')
    return chosen
...
async def _send_daily_wife(bot: Bot, ev: Event, mode: str = 'wife', specified_name: str = ''):
    ...
    if is_transient_draw:
        ...
        if allow_wife_refresh:
            record = await _refresh_daily_wife_record(ev, mode=mode)
        else:
