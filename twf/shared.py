...
__all__ = [
    ...
    '_custom_upload_role_pile_root', '_daily_rng', '_refresh_rng',
    '_download_avatar', '_download_image',
...
def _refresh_rng(ev: Event, user_id: str | int | None = None, salt: str = '') -> random.Random:
    """用于「重复抽取/刷新今日老婆」的随机源。

    与 _daily_rng 不同：额外混入当前毫秒时间戳，保证同一天内反复调用
    不会因为确定性种子而总抽到同一个角色。
    """
    group_key = ev.group_id or 'direct'
    target_user_id = ev.user_id if user_id is None else user_id
    ...
    if salt:
        seed = f'{seed}:{salt}'
    seed = f'{seed}:{int(time.time() * 1000)}'
    logger.debug(f'{LOG_PREFIX} 生成刷新随机数种子: {seed}')
    return random.Random(seed)
